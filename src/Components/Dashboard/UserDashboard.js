...
const UserDashboard = () => {
  ...
  const [usrRoutines, setUsrRoutines] = useState([]);
  ...
  useEffect(() => {
    function getMyRoutines() {
      ...
      axios
        .get(
          `https://onlyfit-backend-staging.herokuapp.com/v1/user/routines?emailAddress=${userEmail}`,
        )
        .then((res) => {

          setUsrRoutines(res.data)     
        })
        .catch((error) => {
  ...
  console.log(usrRoutines,"usrRoutines")
  ...
  if (routines.length > 0) {
    data = routines.map((ele, index) => {
      if(!ele.payment && !(usrRoutines.includes(ele.id))){
          var imageName;
        if (ele.name === "cardio") {
          imageName =
  ...
  if (routines.length > 0) {
    dataPay = routines.map((ele, index) => {
      if(ele.payment && !(usrRoutines.includes(ele.id))){
        var imageName;
        if (ele.name === "cardio") {
          imageName =
